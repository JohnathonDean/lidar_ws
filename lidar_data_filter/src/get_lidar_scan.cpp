...
float lower_threshold = 24000;
float upper_threshold = 26000;
...
void pavoData_Callback(const sensor_msgs::LaserScanConstPtr &scanMsg) {
  ...
  for(int i = 0; i < pointNum; ++i){
      pub.intensities[i] = pub.intensities[i] * 100.0; //将强度值放大100倍
  }
  scan_calculate(pub);
}
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber pavo_subscriber =
      nh.subscribe<sensor_msgs::LaserScan>("pavo_scan", 50, pavoData_Callback);
